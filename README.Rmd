...
**Be aware, due to the large file sizes of the EWSNet model weights (\~230MB), `EWSmethods` does not come bundled with them. Users must download weights using `ewsnet_init()` or `ewsnet_reset()` because interfacing with EWSNet.**
...
```{r rolling_plot, dpi=144, echo = FALSE}
roll_ews$plot
```
...
```{r expanding_plot, dpi=144, echo = FALSE}
exp_ews$plot
```
...
```{r expanding_plot_trait, dpi=144, echo = FALSE}
exp_ews_trait$plot
```
...
```{r expanding_oct_plot, dpi=144, fig.height=6,echo = FALSE}
oct_exp_ews$plot
```
