...
class EWSNet():
    ...
    def __init__(self,ensemble=1, weight_dir=None, prefix="",suffix=".h5"):
        ...
        self.ensemble = random.sample(range(1,25,1),ensemble)
        self.model    = [self.build_model() for _ in self.ensemble]
    ...
    def predict(self,x):
        ...
        predictions = np.array([self.model[i](x)[0] for i in range(1,len(self.ensemble))])
    ...
    def finetune(self,X,y, freeze_feature_extractor=True, learning_rate = 5e-5, batch_size = 512, tune_epochs = 5):
        ...
        for i in range(1,self.ensemble):
    ...
    def load_model(self,weight_dir,prefix,suffix):
        ...
        for i in range(len(self.ensemble)):
            if(os.path.exists("{}/{}{}{}".format(weight_dir,prefix,self.ensemble[i],suffix))):
                self.model[i] = tf.keras.models.load_model("{}/{}{}{}".format(weight_dir,prefix,self.ensemble[i],suffix))
