...
options(timeout = max(300, getOption("timeout"))) #due to possible internet issues, increase the timeout options from 60 seconds to 300
```
...
ewsnet_init(envname = "EWSNET_env", pip_ignore_installed = FALSE, auto = TRUE)
...
| `bypass_reticulate_autoinit()` | Optional set up function to get around quirks of`reticulate`                                                                                  |
