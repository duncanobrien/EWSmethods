...
#find_emo <- system.file(package = "tt")
#if(find_emo == ""){
...
```{r vis_data,dpi=144}
matplot(simTransComms$`1_255_1`[,3:7], type = "l", xlab = "Time", ylab = "Density",  main = "Transitioning five species community")
abline(v=simTransComms$`1_255_1`$inflection_pt,col = "grey60", lty = "dashed")
```
...
```{r mvi2,dpi=144}
mvi_plot_data <- merge(simTransComms$`1_255_1`,as.data.frame(egMVI),by="time") # combine the mvi data with the raw time series
...
eg_uniJI <- uniJI(data = simTransComms$`1_255_1`[,2:3], winsize = 25, E = 3)
```
...
```{r uniJI2,dpi=144}
uniJI_plot_data <- merge(simTransComms$`1_255_1`,eg_uniJI,by="time") # combine the mvi data with the raw time series
...
all_spp_uniJI <- sapply(grep("spp_",colnames(simTransComms$`1_255_1`)),FUN = function(x){
  if(x == 3){
  uniJI(data = simTransComms$`1_255_1`[,c(2,x)], winsize = 25, E = 3)
  }else{
  uniJI(data = simTransComms$`1_255_1`[,c(2,x)], winsize = 25, E = 3)[,2]
  }
...
all_spp_plot_data <- merge(tidyr::pivot_longer(all_spp_uniJI,cols = -time,names_to = "species",values_to = "smap_J"), simTransComms$`1_255_1`[,c("time","inflection_pt")], by = "time") #pivot_longer for easier plotting and merge with the inflection point data
...
Conversely, to compute multivariate form of Jacobian estimation, we can use the function `multiJI()`. `multiJI()` fits a local linear model between all species which predicts the system's future value from the reconstructed state-space. The resulting coefficients of this model are therefore a proxy for the interaction strength between species. 
...
eg_multiJI <- multiJI(data = simTransComms$`1_255_1`[,2:7], winsize = 25)
```
...
```{r multiJI2,dpi=144}
multiJI_plot_data <- merge(simTransComms$`1_255_1`,eg_multiJI,by="time") # combine the mvi data with the raw time series
...
```{r fi}
eg.sost <- t(apply(simTransComms$`1_255_1`[,3:7], MARGIN = 2, FUN = sd)) # define size-of-states using the standard deviation of each species separately. Must be wide format hence t()

egFI <- FI(data = simTransComms$`1_255_1`[,2:7], sost =  eg.sost, winsize = 25,winspace = 1, TL = 90)$FI
```
...
```{r fi2,dpi=144}
fi_plot_data <- merge(simTransComms$`1_255_1`,egFI,by="time") # combine the mvi data with the raw time series
...
Cabezas, H., Campbell, D., Eason, T., et al. (2010). In M. T. Heberling & M. E. Hopton (Eds.), *San Luis Basin sustainability metrics project: A methodology for evaluating regional sustainability* (pp. 119–136). The United States Environmental Protection Agency. [https://nepis.epa.gov](https://nepis.epa.gov/Exe/ZyNET.exe/P100BSKA.TXT?ZyActionD=ZyDocument&Client=EPA&Index=2006+Thru+2010&Docs=&Query=&Time=&EndTime=&SearchMethod=1&TocRestrict=n&Toc=&TocEntry=&QField=&QFieldYear=&QFieldMonth=&QFieldDay=&IntQFieldOp=0&ExtQFieldOp=0&XmlQuery=&File=D%3A%5Czyfiles%5CIndex%20Data%5C06thru10%5CTxt%5C00000029%5CP100BSKA.txt&User=ANONYMOUS&Password=anonymous&SortMethod=h%7C-&MaximumDocuments=1&FuzzyDegree=0&ImageQuality=r75g8/r75g8/x150y150g16/i425&Display=hpfr&DefSeekPage=x&SearchBack=ZyActionL&Back=ZyActionS&BackDesc=Results%20page&MaximumPages=1&ZyEntry=1&SeekPage=x&ZyPURL)
