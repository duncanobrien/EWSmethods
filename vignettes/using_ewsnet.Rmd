...
```{r ewsnet_init, warning = FALSE}
ewsnet_init(envname = "EWSNET_env", pip_ignore_installed = FALSE,  conda_refresh = FALSE) #prepares your session using 'reticulate' and asks to install Anaconda (if no appropriate Python found) and/or a Python environment before activating that environment with the necessary Python packages
```

```{r ewsnet_init_true, echo=FALSE,warning = FALSE}
reticulate::use_condaenv("EWSNET_env") #redundant step necessary for RMarkdown
library(ggplot2)
library(tidyr)
```

If successful, you will see the message `"EWSNET_env successfully found and activated. Necessary Python packages installed"`). If not, please refer to the Troubleshooting section at the end of the tutorial.
...
```{r create_white_data, warning = FALSE,dpi=200, fig.width=5,fig.height=5,fig.cap="Our white noise training data"}
...
ewsnet_prediction <- ewsnet_predict(test_data,noise_type = "W", ensemble = 25, envname = "EWSNET_env") 

ewsnet_prediction
```

### Interpreting EWSNet
As with any machine learning model, interpreting prediction probabilities is difficult. In EWSNet's case, as three possible predictions can be made - _No transition, Smooth transition, Critical transition__ - a chance prediction is one that is approximately 0.33 (1.0 divided by 3 is ~0.33). 
...
For example, using our test data, we could achieve this using this code to identify transitions that may have happened in the past:

```{r expanding_ewsnet, warning = FALSE,dpi=200, fig.width=5,fig.height=5,fig.cap="Changing EWSNet predictions through time"}

expanding_range <- 20:length(test_data) #define the range of indexes to test over

expanding_ewsnet <- data.frame(expanding_range, matrix(NA,ncol=3 )) |>
  `colnames<-`(c("cutoff_ind","No transition",'Smooth','Critical')) #pre-define our out file

for(i in seq_along(expanding_range)){ #use `ewsnet_predict()` for each index 
  ews.tmp <- ewsnet_predict(test_data[1:expanding_range[i]],noise_type = "W", ensemble = 25, envname = "EWSNET_env") 
  
  expanding_ewsnet[i,] <- c(expanding_range[i],ews.tmp$no_trans_prob,ews.tmp$smooth_trans_prob,ews.tmp$critical_trans_prob)
}

ggplot(expanding_ewsnet |> pivot_longer(-cutoff_ind,names_to = "pos_prediction",values_to = "prob"),
                aes(x=cutoff_ind,y=prob)) +
    geom_point(aes(col=pos_prediction)) + theme_bw() +
...
Following finetuning, it may be desirable to reset the model weights to their default. Using the `reset_ewsnet()` function, the user can direct `EWSmethods` to the path of the default weights (these can be redownloaded from the [EWSNet github ]((https://github.com/sahilsid/ewsnet)) which will overwrite the current model weights stored by `EWSmethods`.

```{r ewsnet_reset, eval= FALSE}
reset_ewsnet(new_weight_location = "C:/Users/your_name/Downloads/Pretrained") #for Windows users

reset_ewsnet(new_weight_location = "/Users/your_name/Downloads/Pretrained") #for Mac users
```

You may want to remove `EWSmethods` downloaded Python and environment. You can acheive this using `ewsnet_init()` and the 
`conda_refresh =` argument.

```{r ewsnet_refresh, eval= FALSE}
ewsnet_init(envname = "EWSNET_env",conda_refresh = TRUE)
```
...
| `reset_ewsnet()`| Overwrites the model weights with the default weights downloaded from [EWSNet github](https://github.com/sahilsid/ewsnet) |`bypass_reticulate_autoinit()`| Optional set up function to get around quirks of`reticulate` |                                                                                                                                               |         |
...
If the error remains and your RStudio version >=1.4, you may need to edit your Python interpreter settings in RStudio's preferences. Go to Preferences -> Python and untick "Automatically activate project-level Python environments". Then restart your R session and rerun `ewsnet_init()`.

The final solution we are aware of involves deactivating `reticulate`'s default behaviour in your global R environment settings. Running `bypass_reticulate_autoinit()` will turn off `reticulate`'s to load a Python environment without a user request. Restart your R session and rerun `ewsnet_init()` once ready.
